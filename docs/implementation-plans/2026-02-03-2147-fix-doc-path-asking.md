...
**Design Doc:** docs/design-plans/2026-02-03-2142-fix-doc-path-asking.md **Created:**
2026-02-03
...
Remove directory references from `/plan` and `/build` command Prerequisite sections so
they ask for document paths instead of searching directories.
...
- [x] `/document` works correctly without directory references - Verified: yes, serves as
      pattern to follow

**Patterns to leverage:**

- `/document`'s Prerequisite pattern: simple "ask for path" instruction without directory
  mention
...
**Description:** Remove directory references from both command files' Prerequisite
sections. Simplify to match `/document`'s minimal pattern that correctly triggers asking
behavior.
...
- [x] Run `/build` without arguments — should ask for implementation plan and design doc
      paths
...
This is a minimal text change. The fix works by removing the trigger (directory paths)
that causes Claude to search instead of ask.
