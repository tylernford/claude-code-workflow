...
**Design Doc:** docs/design-plans/2026-01-15-explicit-workflow-rules.md
**Created:** 2026-01-15
...
Add two explicit rules ("Slash commands only" and "One phase per session") to all four workflow command files to enforce core workflow principles.
...
- Existing rule format: numbered list with bold headers (e.g., `1. **Rule name** - Description`)
...
**Description:** Add the "Slash commands only" and "One phase per session" rules to all four workflow command files with phase-appropriate wording.
...
This is a small, focused change. Single task is appropriate because all edits are part of one coherent change and make sense as one commit.
