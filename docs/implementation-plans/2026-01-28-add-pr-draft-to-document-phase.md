...
Add a PR draft (title and description) to the `/document` Phase Complete section. The draft will be generated from the design doc, implementation plan, and build log, giving users copy/paste-ready content for PR creation.
...
- [x] Clear insertion point (after commit checkpoint, before "Feature complete!") - Verified: yes
...
**Description:** Add PR draft generation instructions and output template to the `/document` command. The PR draft appears after the commit checkpoint, before "Feature complete!"
...
1. Add a new subsection (before Phase Complete or inline) with PR draft generation instructions:
...
- The Type → prefix mapping covers common cases. If a design doc has an unlisted Type, Claude should use best judgment or default to `feat:`.
