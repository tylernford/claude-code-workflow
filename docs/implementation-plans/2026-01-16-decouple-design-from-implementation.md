...
Update `/design` and `/plan` prompt language to communicate that Design is standalone
while Plan/Build/Document form a tightly coupled "Implementation Session."
...
**Description:** Update the Phase Complete section to reflect that Design is standalone
and doesn't imply immediate progression to `/plan`.
...
When ready to implement, start a new Claude Code session and run `/plan` to begin an
Implementation Session (Plan → Build → Document).
```

**Done when:** The closing message no longer says "Next: ... Run `/plan` to begin Phase 2"
— instead communicates design is complete and implementation can happen whenever ready.
...
**Description:** Update `/plan` opening to frame it as the start of an Implementation
Session, and add a note to Step 2 about potential time gap since design was written.
...
**Note:** The design document may have been written days or weeks ago. The codebase may
